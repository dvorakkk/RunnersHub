...
// Activities are NOT always Strava numeric IDs: a GPX import gets "gpx-<ts>".
// Accept any safe identifier so flyover share-unlocks and points charging work
// for GPX activities too (this used to reject them with "Invalid activity ID").
function cleanActivityId(v) {
  const s = String(v || '').trim();
  if (!/^[A-Za-z0-9_-]{3,120}$/.test(s)) throw new Error('Invalid activity ID');
  return s;
}
...
async function chargeFlyover(runnerId, activityId, cost) {
  const id = cleanRunnerId(runnerId);
  const aid = cleanActivityId(activityId);
  const wallet = await getWallet(id);
